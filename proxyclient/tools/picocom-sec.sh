...
# Linux host and /dev/tty.usbmodem<asahi mac serial number>3 on macOS host
#
...
echo "Waiting for UART device file to appear (Ctrl+C to abort)..."

while true; do
    while [ ! -n "$SECDEV" ] ; do
        sleep 1
        SECDEV=$(find /dev -maxdepth 1 -type c -regex ".*/tty\.usbmodem[A-Z0-9]\{12\}3" -print -quit)
    done
