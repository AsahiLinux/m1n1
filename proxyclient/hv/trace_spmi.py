...
class SPMITracer(ADTDevTracer):
    ...
    def device_name(self, device_id):
        if device_id in self.state.devices and self.state.devices[device_id].name:
            return self.state.devices[device_id].name
        return f"0x{device_id}?"
...
class ACE3SPMIDevTracer(SPMIDevTracer):
    ...
    def flush(self):
        if self.last_opcode:
            self.log(f"0x{self.reg:02x} {self.last_opcode}: {self.data.hex()}")
            self.data = b""
            self.state = 'idle'
            self.last_opcode = None
...
SPMITracer = SPMITracer._reloadcls()
...
spmi_tracer = SPMITracer(hv, "/arm-io/nub-spmi-a0", verbose=0)
