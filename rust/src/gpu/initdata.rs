...
pub unsafe extern "C" fn rust_fill_gpu_initdata(
    ...
    let csafr = if hwcfg.has_csafr {
        ...
        for i in 0..ins.n_perf_states_cs {
            ...
            perf_states_cs.push(hw::PState {
                volt_mv: volts,
                pwr_mw: 0,
                freq_hz: unsafe { ins.pstates_cs.offset(i as isize).read().freq },
            });
        }
        for i in 0..ins.n_perf_states_afr {
            ...
            perf_states_afr.push(hw::PState {
                volt_mv: volts,
                pwr_mw: 0,
                freq_hz: unsafe { ins.pstates_afr.offset(i as isize).read().freq },
            });
        }
    ...
    let pwrcfg = hw::PwrConfig {
        ...
        perf_base_pstate: ins.base_pstate,
        perf_max_pstate: perf_states.len() as u32 - 1,

        perf_states,
        csafr,
    };
    let gpu_rev_id = if let Some(r) = raw_rev_to_id(ins.gpu_rev_raw) {
        r
    } else {
        println!("Unknown gpu revision: {}", ins.gpu_rev_raw);
        return -1;
    };
    let dyncfg = DynConfig {
        uat_ttb_base: ins.uat_ttb_base,
        pwr: pwrcfg,
        id: GpuIdConfig {
            gpu_rev_id,
            num_cores: ins.num_cores,
            num_frags: ins.num_cores,
        },
    };
    unsafe {
        match (
