 ...
 * tunable struct and that size is in bytes instead of bits.
...
static int dt_copy_acio_tunables(const char *adt_path, const char *dt_alias)
{
    ...
    fdt_setprop(dt, fdt_node, "apple,drom", drom_blob, drom_len);

    for (size_t i = 0; i < sizeof(atc_tunables) / sizeof(*acio_tunables); ++i) {
...
err:
    fdt_delprop(dt, fdt_node, "apple,drom");
