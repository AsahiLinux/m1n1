...
static tps6598x_dev_t *hpm_init_spmi(spmi_dev_t *spmi, const char *hpm_path)
{
...
static int usb_init_spmi(const char *spmi_path)
{
    ...
    ADT_FOREACH_CHILD(adt, node)
    {
        ...
        tps6598x_dev_t *tps = hpm_init_spmi(spmi, hpm_path);
        if (!tps) {
...
static tps6598x_dev_t *hpm_init_i2c(i2c_dev_t *i2c, const char *hpm_path)
{
...
static int usb_init_i2c(const char *i2c_path)
{
    ...
    ADT_FOREACH_CHILD(adt, node)
    {
        ...
        tps6598x_dev_t *tps = hpm_init_i2c(i2c, hpm_path);
        if (!tps) {
            printf("usb: failed to init %s\n", name);
            continue;
        }
...
void usb_init(void)
{
    ...
    if (adt_is_compatible(adt, 0, "J575dAP") && usb_init_spmi("/arm-io/nub-spmi-a2") < 0)
        return;
    if (usb_init_spmi("/arm-io/nub-spmi-a0") < 0)
        return;
    if (usb_init_spmi("/arm-io/nub-spmi-a1") < 0)
        return;
...
void usb_spmi_restore_irqs(const char *spmi_path, bool force)
{
    char hpm_path[MAX_HPM_PATH_LEN];
...
void usb_hpm_restore_irqs(bool force)
{
    ...
    if (adt_is_compatible(adt, 0, "J575dAP"))
        usb_spmi_restore_irqs("/arm-io/nub-spmi-a2", force);
    usb_spmi_restore_irqs("/arm-io/nub-spmi-a0", force);
    usb_spmi_restore_irqs("/arm-io/nub-spmi-a1", force);
